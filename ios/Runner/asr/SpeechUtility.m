...
+ (NSError *)raiseErrorWithCode:(NSInteger)code message:(NSString *)errorMessage reason:(NSString *)reasonMessage suggestion:(NSString *)suggestionMessage{
    ...
    return [NSError errorWithDomain:@"SPEECHWS"
                                         code:code
                                     userInfo:userInfo];
    }
    else
    {
        return [NSError errorWithDomain:@"SPEECHWS" code:code userInfo:NULL];
    }
}
...
+ (NSError*)raiseErrorWithMessage:(NSString*) errorMessage{
    return [SpeechUtility raiseErrorWithCode:WEBSOCKETS_ERROR_CODE
