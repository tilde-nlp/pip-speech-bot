...
// tildes balss specific stuff
@property BOOL hasAdaptationStateBeenReceived;
...
- (void) connect:(STTConfiguration*)config headers:(NSDictionary*)headers  {
    ...
    self.hasAdaptationStateBeenReceived = NO;
...
- (void)webSocket:(SRWebSocket *)webSocket didFailWithError:(NSError *)error;
{
    ...
    if ([self.reconnectAttempts intValue] < 1) { //E.K.
...
- (void)webSocket:(SRWebSocket *)webSocket didReceiveMessage:(id)json;
{
    ...
    if ([object isKindOfClass:[NSDictionary class]]) {
        ...
        if ([results objectForKey:@"result"] != nil) {
            NSArray *resultsArr = [results objectForKey:@"result"];
        ...
        if ([results objectForKey:@"adaptation_state"] != nil) {
            // if we see adaptation_state, this means socket will be closed
            // we set a flag that we use when handling websocket being closed
            // to distinguish between actual erroneous websocket disconnections
            // and expected ones after server has finished processing with no results, i.e., NO_SPEECH case
            self.hasAdaptationStateBeenReceived = YES;
        }
    } else {
...
- (void)webSocket:(SRWebSocket *)webSocket didCloseWithCode:(NSInteger)code reason:(NSString *)reason wasClean:(BOOL)wasClean;
{
    ...
    if(self.hasDataBeenSent == NO && self.hasAdaptationStateBeenReceived == NO){
    ...
    if (self.hasAdaptationStateBeenReceived == NO) {
        // if we have received adaptation state, then socket closed with whatever code is fine...
        errorMessage = [SpeechUtility findUnexpectedErrorWithCode: code];
    }
